...
#---------------------------------------------------------------------------------
LIBS	:= 	-lfilesystem -lfat -ldswifi9 -lnds9 -lm 
...
.PHONY: $(SOURCEARM7)/pxtone7.elf

$(OUTPUT).nds	: 	$(OUTPUT).elf $(SOURCEARM7)/pxtone7.elf
$(OUTPUT).nds	: 	$(shell find $(TOPDIR)/$(NITRODATA))
$(OUTPUT).elf	:	$(OFILES)

$(SOURCEARM7)/pxtone7.elf:
...
ifeq ($(DSI), 1)
%.nds: %.elf
	$(SILENTCMD)ndstool -c $@ -9 $< -7 $(TOPDIRREAL)/$(SOURCEARM7)/pxtone7.elf -b $(GAME_ICON) "$(GAME_TITLE);$(GAME_SUBTITLE1);$(GAME_SUBTITLE2)" $(_ADDFILES)
	echo built ... $(notdir $@)
else
%.nds: %.elf
	$(SILENTCMD)ndstool -h 0x200 -c $@ -9 $< -7 $(TOPDIRREAL)/$(SOURCEARM7)/pxtone7.elf -b $(GAME_ICON) "$(GAME_TITLE);$(GAME_SUBTITLE1);$(GAME_SUBTITLE2)" $(_ADDFILES)
	echo built ... $(notdir $@)
endif
