...
bool pxtnService::_moo_PXTONE_SAMPLE( int smp_num )
{
	if( !_moo_b_init ) return false;
	...
	// --------------
	// increments..
	_moo_smp_count += smp_num;

	_moo_time_pan_index = ( _moo_time_pan_index + smp_num ) & ( pxtnBUFSIZE_TIMEPAN - smp_num );

	for( int32_t u = 0; u < _unit_num;  u++ )
	{
		int32_t  key_now = _units[ u ]->Tone_Increment_Key(smp_num);
		_units[ u ]->Tone_Increment_Sample( _moo_freq->Get2( key_now ) *_moo_smp_stride, smp_num );
	}
...
bool pxtnService::Moo( void* p_buf, int32_t  size )
{
...

	{
		...
		if( !_moo_PXTONE_SAMPLE( smp_num ) ){ _moo_b_end_vomit = true; }
